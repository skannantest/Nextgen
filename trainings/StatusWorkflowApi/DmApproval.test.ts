...
test('Create Engagement API for DM approval', async ({ request }) => {
...
test('DM approval API', async ({ request }) => {
  const response = await request.post(
  ...
  expect(isApproved).toBe('The engagement was approved successfully');
  console.log('----------------------------------------------------------------');
